...
// --- PATCH START: Filter Logic for Agents ---
...
const patchedFilterTypes = computed(() => {
    ...
    if (strictlyRestricted) {
        ...
        // Find the 'status' attribute definition
        const statusType = types.find(t => t.attributeKey === 'status');
  ...
  >
    ...
    <ul class="grid gap-4 list-none">
      <template v-for="(filter, index) in filters" :key="filter.id">
        <!-- PATCH START: Use patchedFilterTypes -->
        <ConditionRow
          v-if="index === 0"
          ref="conditionsRef"
          ...
          :filter-types="patchedFilterTypes"
